...
- 机器人启动时会自动设置简介：用户点开聊天、未按 Start 前就能看到一句话说明用途。
...
- 编辑同步：`copy` 模式下，任一方编辑消息后，另一方的复制品会自动同步更新（文本、图片/视频/动画/音频/文件的标题或内容均可；贴纸、语音、投票等 Telegram 本身不允许编辑的类型除外）。`forward` 模式是转发消息，无法编辑。消息删除没有事件，删除无法同步。
- 删除话题：Telegram 不会向机器人推送“话题已删除”事件。推荐管理员在要删除的话题内发送 `/deletetopic yes` 让机器人删除，机器人会同时解绑该话题的用户、移出白名单并推送“会话结束”通知（即时且可靠）。如果仍用 Telegram 界面直接删除，则只能等该用户下次私聊发消息时自动检测清理（发送失败触发），管理员也可手动 `/disallow` 立即清理。
...
| 命令 | 说明 |
|---|---|
| `/bind <用户ID或@用户名>` | 在当前群/Topic 中绑定该用户；若用户不在白名单中会自动加入，之后该用户的消息只和这个目标互通 |
| `/bind <用户ID或@用户名> <群ID> [TopicID]` | 绑定到指定群；TopicID 留空表示整个群/频道 |
| `/deletetopic yes` | 在当前话题内执行：永久删除该话题并自动解绑其用户、移出白名单、推送会话结束通知（需机器人有“删除消息”权限） |
...
| `/disallow <用户ID或@用户名>` | 把用户移出白名单，并同时解除他/她的全部绑定（所有群/Topic/频道）；仅当该用户确实在白名单中时，才推送会话结束通知 |
| `/leave`（群内） | 让机器人退出当前群 |
| `/leave <群ID>`（私聊管理员） | 远程让机器人退出指定群，并清理该群绑定 |
| `/groups`（私聊管理员） | 查看机器人记录到的所有群（群 ID + 群名），可配合 `/leave` 使用 |
| `/forgetgroup <群ID>`（私聊管理员） | 手动把某个群从 `/groups` 记录中移除 |
...
注意：Telegram 没有“查询机器人加入了哪些群”的接口，`/groups` 只能列出**机器人收到过消息/入群通知**的群；新部署后，需要群里有人发消息才会被记录。机器人退群、被移出或群被删除时，会自动从记录中移除；历史残留可用 `/forgetgroup <群ID>` 清理。
