...
- 删除话题：Telegram 不会向机器人推送“话题已删除”事件，因此机器人无法在删除瞬间自动清理。改为：被删话题对应的外部用户下次私聊发消息时，机器人会检测到发送失败（`message thread not found`），自动解除该用户的绑定、移出白名单，并推送“会话结束”通知。管理员也可以立即手动执行 `/disallow` 完成同样的清理。
