...
| `/allow @用户` | 加入白名单 |
| `/disallow @用户` | 移出白名单并清除全部绑定，同时向用户推送会话结束通知 |
| `/bind @用户` | 把用户绑定到当前 Topic |
...
> 删除某个 Topic 后，机器人会自动解绑该 Topic 下的所有用户、把他们移出白名单，并推送“会话结束”通知，无需手动处理。
...
- `/disallow @用户名` 移出白名单并清除全部绑定，同时向用户推送会话结束通知。
